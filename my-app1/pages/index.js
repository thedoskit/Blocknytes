...
export default function Home() {
  ...
  // `bntcBalance` is the amount of BNTC tokens held by the users account
  const [bntcBalance, setBNTCBalance] = useState(zero);
  ...
  // removeEther is the amount of `Ether` that would be sent back to the user based on a certain number of `BNTC` tokens
  const [removeEther, setRemoveEther] = useState(zero);
  // removeCD is the amount of `Crypto Dev` tokens that would be sent back to the user based on a certain number of `BNTC` tokens
  // that he wants to withdraw
  const [removeCD, setRemoveCD] = useState(zero);
  // amount of BNTC tokens that the user wants to remove from liquidity
  const [removeBNTCTokens, setRemoveBNTCTokens] = useState("0");
  ...
  const getAmounts = async () => {
    try {
      ...
      // get the amount of `Crypto Dev` BNTC tokens held by the user
      const _bntcBalance = await getBNTCTokensBalance(provider, address);
  ...
  /**
   * _removeLiquidity: Removes the `removeBNTCTokensWei` amount of BNTC tokens from
   * liquidity and also the calculated amount of `ether` and `CD` tokens
   */
  const _removeLiquidity = async () => {
    try {
      const signer = await getProviderOrSigner(true);
      // Convert the BNTC tokens entered by the user to a BigNumber
      const removeBNTCTokensWei = utils.parseEther(removeBNTCTokens);
      setLoading(true);
      // Call the removeLiquidity function from the `utils` folder
      await removeLiquidity(signer, removeBNTCTokensWei);
      setLoading(false);
      await getAmounts();
      setRemoveCD(zero);
      setRemoveEther(zero);
   ...
   */
  const _getTokensAfterRemove = async (_removeBNTCTokens) => {
    try {
      const provider = await getProviderOrSigner();
      // Convert the BNTC tokens entered by the user to a BigNumber
      const removeBNTCTokenWei = utils.parseEther(_removeBNTCTokens);
  ...
  */
      ...
      return (
        <div>
          <div className={styles.description}>
            ...
            <br />
            {utils.formatEther(bntcBalance)} Crypto Dev BNTC tokens
          </div>
          <div>
            ...
            <div>
              <input
                type="number"
                placeholder="Amount of BNTC Tokens"
                onChange={async (e) => {
                  setRemoveBNTCTokens(e.target.value || "0");
                  // Calculate the amount of Ether and CD tokens that the user would receive
                  // After he removes `e.target.value` amount of `BNTC` tokens
  ...
  return (
    <div>
      <Head>
        <title>Blocknytes</title>
        <meta name="description" content="Whitelist-Dapp" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to Blocknytes Exchange!</h1>
          <div className={styles.description}>
      ...
      <footer className={styles.footer}>
        Made with &#10084; By The Blocknytes Group
      </footer>
    </div>
  );
}