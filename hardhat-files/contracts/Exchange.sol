...
contract Exchange is ERC20 {
    ...
    // Exchange is inheriting ERC20, because our exchange would keep track of Crypto Dev BNTC tokens
    constructor(address _CryptoDevtoken) ERC20("Blocknytes Token", "BNTC") {
    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        */
        if(cryptoDevTokenReserve == 0) {
            ...
            // Take the current ethBalance and mint `ethBalance` amount of BNTC tokens to the user.
            ...
            */
            ...
            // The amount of BNTC tokens that would be sent to the user should be propotional to the liquidity of
            // ether added by the user
            // Ratio here to be maintained is ->
            // (BNTC tokens to be sent to the user (liquidity)/ totalSupply of BNTC tokens in contract) = (Eth sent by the user)/(Eth reserve in the contract)
            // by some maths -> liquidity =  (totalSupply of BNTC tokens in contract * (Eth sent by the user))/(Eth reserve in the contract)
    ...
    */
    function removeLiquidity(uint _amount) public returns (uint , uint) {
        ...
        // = (amount of BNTC tokens that user wants to withdraw) / (total supply of BNTC tokens)
        // Then by some maths -> (Eth sent back to the user)
        // = (current Eth reserve * amount of BNTC tokens that user wants to withdraw) / (total supply of BNTC tokens)
        ...
        // = (current Crypto Dev token reserve * amount of BNTC tokens that user wants to withdraw) / (total supply of BNTC tokens)
        uint cryptoDevTokenAmount = (getReserve() * _amount)/ _totalSupply;
        // Burn the sent `BNTC` tokens from the user's wallet because they are already sent to
        // remove liquidity
